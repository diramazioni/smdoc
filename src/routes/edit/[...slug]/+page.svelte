<script lang="ts">
  ...
  let directoryValue = $state(data.slug.split('/').slice(0, -1).join('/'));
  let autoSaveDialog = $state(false);
  let interval = $state();

  // Create slug from title and directory
  let slug = $derived.by(() => {
    const slugTitle = slugify(titleValue);
    return directoryValue ? `${directoryValue}/${slugTitle}` : slugTitle;
  });
  ...
  async function handleSave(event) {
    ...
    frontmatterData.append('directory', directoryValue);
    ...
    formData.append('directory', directoryValue);
  ...
  function clearFields() {
    titleValue = descriptionValue = directoryValue = "";
...
<form method="POST" action="/edit/{slug}?/frontmatter" class="w-full bg-muted p-2 -mt-2">
  <div class="flex max-w-full items-center m-4 space-x-3">
    ...
    <Input type="text" name="directory" placeholder="e.g. prodotti" bind:value={directoryValue} class="w-20"/>
    ...
    <Input type="text" name="slug_view" disabled value={slug} class="w-30"/> 
