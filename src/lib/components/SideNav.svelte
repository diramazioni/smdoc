<script lang="ts">
  ...
  let { class: className = undefined, items } = $props<{
...
<div class="relative">
  ...
  >
    ...
    {:else}
      <!-- <Menu class="h-6 w-6" /> -->
      <div
        class="bg-green-500 hover:bg-green-700 text-white font-medium py-2 px-4 rounded-md text-sm focus:outline-hidden focus:ring-2 focus:ring-green-300"
      >
  ...
  {#if isSidebarOpen}
    <button
      ...
      onclick={toggleSidebar}>s</button
    >
  {/if}
</div>
